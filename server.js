...
const fetchPromise = typeof globalThis.fetch === "function"
...
async function uploadZendeskFile(fetchFn, { zendeskDomain, auth, file }) {
...
app.post("/api/zendesk", async (req, res) => {
  ...
  try {
    const zendeskFetch = await fetchPromise;
    const uploadTokens = [];
    ...
    console.log(`Ticket Zendesk cree : #${ticketId}`);
